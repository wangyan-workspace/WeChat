...
1 获取用户的收货地址
  1 绑定点击事件
  2 调用小程序内置 api  获取用户的收货地址  wx.chooseAddress =>如果用户不小心点击取消，下次无法获取地址（行不通）
...
Page({
  ...
  onShow() {
    ...
    // 2.计算全选
    // every 数组方法 会遍历 会接收一个回调函数，那么每一个回调函数都返回true那么every方法的返回值为true
    // 只要有一个回调函数返回了false那么不在循环执行，直接返回false
    // 空数组 调用every，返回值就是true(没有商品也会被选中，这个就有点问题)
  ...
  async handleItemNumEdit(e) {
    ...
    if (cart[index].num === 1 && operation === -1) {
      ...
      if (res.confirm) {
        cart.splice(index, 1); //删除数组中数量为0的商品的信息
        this.setCart(cart);
      }
    } else {
      // 4  进行修改数量
